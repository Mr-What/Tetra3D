...
%
% For humans, we define :
%          tower_radius  (mm)
%          tower_angle (usually [210,330,90])
%          arm_length
%          tilt_zenith  (towards origin)
%          tilt_tangential (towards left as tower facing origin)
%          endstop_dist (distance from z=0 plane to location of arm axis at endstop)
%
...
%    arm[3]    -- length of arm crom carriage to effector
%
...
% longer variable names for definition in a [printer] section
%
function tp=getTetraParams(p0)
    if isfield(p0,"tower_radius")
...
function p = tetraDefinitionParams(k)
    p.arm_length = k.arm;
    p.tower_radius = [150,150,150];
    p.tower_angle = [210,330,90];
    p.tilt_zenith = [0,0,0];
    ...
    for n=1:3
        ...
        r = norm(q);
        p.tower_radius(n) = r;
        p.tower_angle(n) = atan2d(q(2), q(1));
        rHat = -q / norm(q);
        tHat =  [-q(2), q(1), 0];
        d = k.dir(n,:);
        p.tilt_zenith(n) = atan2d(dot(rHat,d), d(3) );
