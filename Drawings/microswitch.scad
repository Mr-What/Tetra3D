...
module microswitchHoles(fuzz=0,diam=2.5,sep=dHole,len=7)
    pairX(sep/2) rotate([90, 0, 0])
        cylinder(d=diam+fuzz, h=len, center=true, $fn=24);
...
/// old model, less detail, not centered on switch tab
module microswitch1() {
  difference() {
    union() {
      translate([0, 0, 2.5])
