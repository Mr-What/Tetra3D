...
%translate([-9.65,35.3,-3]) rotate([90,0,90]) import("microprobe_v2_mockup.stl");
...
module microprobeFrame() union() { kinematicProbeFrame();
    hull() {
        translate([0,-10,14]) pairX(11) sphere(r=4,$fn=RES);
        translate([0,  3,14]) pairX(9) sphere(r=4,$fn=RES);
        for(a=[90:120:355]) rotate(a)
            translate([16,0,23]) cylinder(r=3,h=1,$fn=RES);
    }
}

module kinMicroprobeV2mount() union() {
    difference() { microprobeFrame();
        
        #kinMountBalls(nf=RES);
        
        // clear area around switch
        //translate([0,-1.8,9.1]) cube([22+.3,16,18.2], center=true);
        
        mirror([1,0,0]) microprobeEnvelope();
                        microprobeEnvelope();

        // wire hole
        hull() translate([0,-7.8,12]) pairX(6) cylinder(r=3,h=20,$fn=RES/2); 
        ...
        translate([0,-2.75,0]) pairX(17/2-.04) cylinder(r=1.4,h=40,$fn=RES/2);
